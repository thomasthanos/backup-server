            ...
            if os.path.exists(d) and os.path.isdir(d):
                try:
                    ...
                    try:
                        import shutil
                        shutil.rmtree(d)
                        logger.info(f"Φυσικός φάκελος διαγράφηκε (με shutil): {d}")
                        deleted_physical = True
                        break
                    except Exception as shutil_error:
                        logger.error(f"Σφάλμα διαγραφής φακέλου {d}: {shutil_error}")
                        continue
        
        if not deleted_physical:
            logger.warning(f"Δεν βρέθηκε φυσικός φάκελος για διαγραφή. Candidate paths: {candidate_dirs}")
